...
namespace APIdata.Model
{
    public class AllCurentObjects
    {
        ...
        public async Task GetFromAPIByDate(DateTime date)
        {
            HttpClient http = new HttpClient();
            string path = Environment.GetFolderPath(Environment.SpecialFolder.Personal);
            string filename = Path.Combine(path, "lastObject.txt");
            try
            {
                string url = "https://api.nasa.gov/neo/rest/v1/feed?start_date=" + date.ToString("yyyy-MM-dd") + "&end_date=" + date.ToString("yyyy-MM-dd") + "&api_key=d5m1LesAjClwYjHv5pDC572UnkZOhKls0jOomejg";
                HttpResponseMessage response = await http.GetAsync(url, HttpCompletionOption.ResponseContentRead);
                string res = await response.Content.ReadAsStringAsync();
                JObject jo = JObject.Parse(res);
                if (File.Exists(filename))
                ...
                {
                    Telesa tl = new Telesa();
                    tl.Name = Convert.ToString(jo["near_earth_objects"][date.ToString("yyyy-MM-dd")][i]["name"]);
                    tl.IsDangerous = Convert.ToBoolean(jo["near_earth_objects"][date.ToString("yyyy-MM-dd")][i]["is_potentially_hazardous_asteroid"]);
                    tl.KMPerHour = Convert.ToDouble(jo["near_earth_objects"][date.ToString("yyyy-MM-dd")][i]["close_approach_data"][0]["relative_velocity"]["kilometers_per_hour"]);
                    tl.MissDistance = Convert.ToDouble(jo["near_earth_objects"][date.ToString("yyyy-MM-dd")][i]["close_approach_data"][0]["miss_distance"]["kilometers"]);
                    tl.CloseApproachDate = Convert.ToDateTime(jo["near_earth_objects"][date.ToString("yyyy-MM-dd")][i]["close_approach_data"][0]["close_approach_date"]);
                    AllObjects.Add(tl);
                }
            }
            catch (HttpRequestException ex)
